...
def main(args):
    ...
    metrics_base = collect_metrics(dir_base)
    metrics_reg = collect_metrics(dir_reg)
    ler_nolm_base = metrics_base[('LER', 'beam', '1000_min', 'lm_n')].to_frame('LER (no spell-checking)')
    ler_lm_base = metrics_base[('LER', 'beam', '1000_min', 'lm_y')].to_frame('LER (with spell-checking)')
    ler_nolm_reg = metrics_reg[('LER', 'beam', '1000_min', 'lm_n')].to_frame('LER (regularized model, no spell-checking')
    ler_lm_reg = metrics_reg[('LER', 'beam', '1000_min', 'lm_y')].to_frame('LER (regularized model), with spell-checking)')
