...
from os.path import realpath, join, dirname, isdir, exists, abspath
...
if __name__ == '__main__':
    ...
    parser.add_argument('cwd', type=str, nargs='?', default='.',
                        help='(optional) directory to serve from (default: current directory)')
    parser.add_argument('port', type=int, nargs='?', default=8000, help='(optional) port to use (default: 8000)')
    args = parser.parse_args()

    source_dir = abspath(join(dirname(realpath(__file__)), args.cwd))
    print(f'serving from {source_dir} on port {args.port}')
    os.chdir(source_dir)
