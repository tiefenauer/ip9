...
def extract_segments(target_dir, corpus_id, corpus, synthesize=False, max_audio_length=0, override=False):
    train_set = list(corpus.train_set(numeric=args.include_numeric))
    dev_set = list(corpus.dev_set(numeric=args.include_numeric))
    test_set = list(corpus.test_set(numeric=args.include_numeric))
